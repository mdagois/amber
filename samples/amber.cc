...
struct Options {
  ...
  bool enable_performance_layers = false;
...
const char kUsage[] = R"(Usage: amber [options] SCRIPT [SCRIPTS...]
 ...
 options:
  ...
  --enable-perf-layers      -- Enable performance layers.
  -h                        -- This help text.
)";
...
bool ParseArgs(const std::vector<std::string>& args, Options* opts) {
  for (size_t i = 1; i < args.size(); ++i) {
    ...
    } else if(arg == "--enable-perf-layers") {
      opts->enable_performance_layers = true;
...
int main(int argc, const char** argv) {
  ...
  amber::Result r = config_helper.CreateConfig(
      ...
      options.disable_validation_layer, options.enable_performance_layers,
